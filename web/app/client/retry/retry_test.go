...
func TestApply(t *testing.T) {
	...
	assert.Equal(t, uint(100), config.MaxAttemptTimes)
	assert.Equal(t, time.Second, config.Delay)
	assert.Equal(t, time.Second, config.MaxDelay)
	assert.Equal(t, time.Second, Delay(0, nil, &config))
	assert.Equal(t, time.Second, config.MaxJitter)
}
...
func TestPolicy(t *testing.T) {
	...
	dur = Delay(0, nil, &config)
	assert.Equal(t, config.MaxDelay, dur)
