...
import (
	...
	"github.com/caiflower/common-tools/web/app/client/discovery"
...
func (b *BalancerFactory) refresh() {
	for range time.Tick(b.opts.RefreshInterval) {
		b.cache.Range(func(key, value interface{}) bool {
			...
			if err != nil {
				logger.Warn("resolver refresh failed, key=%s error=%s", key, err.Error())
				return true
			}
...
func (b *BalancerFactory) GetInstance(ctx context.Context, req *protocol.Request) (discovery.Instance, error) {
	...
	if ins == nil {
		logger.Error("null instance. serviceName: %s, options: %v", string(req.Host()), req.Options())
