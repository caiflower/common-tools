...
func (sc *serverConn) rejectConn(err ErrCode, debug string) {
	if VerboseLogs {
		logger.Info("HTTP2: server rejecting conn: %v, %s", err, debug)
	}
...
func (sc *serverConn) serve() {
	...
	if VerboseLogs {
		logger.Info("HTTP2: server connection from %v", sc.conn.RemoteAddr())
	}
	...
	for {
		...
		case msg := <-sc.serveMsgCh:
			...
			case *serverMessage:
				...
				case settingsTimerMsg:
					logger.Error("HTTP2: Timeout waiting for SETTINGS frames from %v", sc.conn.RemoteAddr())
					return
				case idleTimerMsg:
					if VerboseLogs {
						logger.Info("HTTP2: Connection is idle")
					}
					sc.goAway(ErrCodeNo)
				case shutdownTimerMsg:
					if VerboseLogs {
						logger.Info("HTTP2: GOAWAY close timer fired; closing conn from %v", sc.conn.RemoteAddr())
					}
					return
		...
		if sc.queuedControlFrames > sc.srv.maxQueuedControlFrames() {
			if VerboseLogs {
				logger.Info("HTTP2: too many control frames in send queue, closing connection")
			}
			return
		}
...
func (sc *serverConn) readPreface() error {
	...
	case err := <-errc:
		if err == nil {
			if VerboseLogs {
				logger.Info("HTTP2: client %v said hello", sc.conn.RemoteAddr())
			}
		}
		return err
	}
...
func (sc *serverConn) processFrameFromReader(res readFrameResult) bool {
	...
	if err != nil {
		...
		f := res.f
		if VerboseLogs {
			logger.Info("HTTP2: read frame %v", summarizeFrame(f))
		}
	...
	case ConnectionError:
		logger.Error("HTTP2: connection error from %v: %v", sc.conn.RemoteAddr(), ev)
		sc.goAway(ErrCode(ev))
		return true // goAway will handle shutdown
	default:
		if res.err != nil {
			if VerboseLogs {
				logger.Info("HTTP2: closing client connection; error reading frame from client %s: %v", sc.conn.RemoteAddr(), err)
			}
		} else {
			logger.Error("HTTP2: closing client connection: %v", err)
		}
		return false
	}
}
...
func (sc *serverConn) processFrame(f Frame) error {
	...
	default:
		if VerboseLogs {
			logger.Info("HTTP2: ignoring frame: %v", f.Header())
		}
		return nil
	}
}
...
func (sc *serverConn) processSetting(s Setting) error {
	...
	if VerboseLogs {
		logger.Info("HTTP2: processing setting %v", s)
	}
	switch s.ID {
	...
	default:
		...
		// ignore that setting."
		if VerboseLogs {
			logger.Info("HTTP2: ignoring unknown setting %v", s)
		}
	}
	return nil
}
...
func (sc *serverConn) processGoAway(f *GoAwayFrame) error {
	...
	if f.ErrCode != ErrCodeNo {
		logger.Error("HTTP2: received GOAWAY %+v, starting graceful shutdown", f)
	} else {
		if VerboseLogs {
			logger.Info("HTTP2: received GOAWAY %+v, starting graceful shutdown", f)
		}
	}
	sc.startGracefulShutdownInternal()
...
func (sc *serverConn) processHeaders(f *MetaHeadersFrame) error {
	...
	if f.HasPriority() {
		if err := checkPriority(f.StreamID, f.Priority); err != nil {
			//if st.reqCtx.IsEnableTrace() {
			//	traceinfo.Record(st.reqCtx.GetTraceInfo(), stats.ReadHeaderFinish, err)
			//}
			return err
		}
	...
	st.handler = sc.engine.Core.Serve
	if f.Truncated {
...
func (sc *serverConn) newStream(id, pusherID uint32, state streamState) *stream {
	...
	if sc.engine.EnableTrace {
		...
		//traceinfo.Record(reqCtx.GetTraceInfo(), stats.ReadHeaderStart, nil)
	}
...
func (sc *serverConn) runHandler(rw *responseWriter, reqCtx *webctx.RequestCtx, handler app.HandlerFunc) {
	...
	defer func() {
		...
		defer func() {
			if reqCtx.IsEnableTrace() {
				//traceinfo.Record(reqCtx.GetTraceInfo(), stats.ServerHandleFinish, err)
			}
		}()
		...
		if err != nil {
			...
			logger.Error("HTTP2: write response body error when serving %v: %v", sc.conn.RemoteAddr(), err)
			return
		}
	...
	if reqCtx.IsEnableTrace() {
		//traceinfo.Record(reqCtx.GetTraceInfo(), stats.ServerHandleStart, nil)
	}
	handler(reqCtx)
...
func (sc *serverConn) startPush(msg *startPushRequest) {
	...
	allocatePromisedID := func() (uint32, error) {
		...
		go sc.runHandler(rw, promised.reqCtx, sc.engine.Core.Serve)
