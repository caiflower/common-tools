...
func TestDataBufferAllocation(t *testing.T) {
	...
	testDataBuffer(t, wantRead.Bytes(), func(t *testing.T) *dataBuffer {
		b := &dataBuffer{buf: bytebufferpool.Get()}
		for _, p := range writes {
		...
		want = append(want, bytes.Repeat([]byte("c"), 8*1024)...)
		want = append(want, bytes.Repeat([]byte("d"), 16*1024)...)
		want = append(want, bytes.Repeat([]byte("e"), 32*1024)...)

		if !reflect.DeepEqual(b.buf.B, want) {
...
func TestDataBufferAllocationWithExpected(t *testing.T) {
	...
	testDataBuffer(t, wantRead.Bytes(), func(t *testing.T) *dataBuffer {
		...
		want = append(want, bytes.Repeat([]byte("c"), 15*1024)...)
		want = append(want, bytes.Repeat([]byte("d"), 2*1024)...)
		want = append(want, bytes.Repeat([]byte("e"), 1*1024)...)
		if !reflect.DeepEqual(b.buf.B, want) {
			t.Errorf("dataBuffer.chunks\ngot:  %s\nwant: %s", b.buf.B, want)
		}
		return b
