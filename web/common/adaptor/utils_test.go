...
import (
	...
	bytestr "github.com/caiflower/common-tools/web/common/bytestr"
...
func TestMethodStr(t *testing.T) {
	...
	assert.Equal(t, true, diff < 50, diff)
}
...
func TestParseHTTPVersion(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, major, 2)
	assert.Equal(t, minor, 0)

	// Test HTTP/3.1
	major, minor, err = parseHTTPVersion("HTTP/3.1")
	assert.Nil(t, err)
	assert.Equal(t, major, 3)
	assert.Equal(t, minor, 1)
