...
func TestReadFromHasBufferAvailable(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, len(rawData), int(l))
	...
	err2 := conn.Flush()
	assert.Nil(t, err2)
	assert.Equal(t, string(preData)+rawData+string(tailData), c.buffer.String())
}
...
func TestInitializeConn(t *testing.T) {
	...
	assert.Equal(t, errors.New("conn: write deadline not supported"), conn.SetWriteDeadline(time.Time{}))
	assert.Equal(t, errors.New("conn: read deadline not supported"), conn.SetReadTimeout(time.Duration(1)*time.Second))
	assert.Equal(t, errors.New("conn: read deadline not supported"), conn.SetReadTimeout(time.Duration(-1)*time.Second))
	assert.Equal(t, errors.New("conn: method not supported"), conn.Close())
	assert.Equal(t, &mockAddr{network: "tcp", address: "192.168.0.10:80"}, conn.LocalAddr())
	assert.Equal(t, &mockAddr{network: "tcp", address: "192.168.0.20:80"}, conn.RemoteAddr())
}
...
func TestHandleSpecificError(t *testing.T) {
	...
	assert.Equal(t, true, conn.HandleSpecificError(syscall.EPIPE, ""))
}
