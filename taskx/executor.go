...
func RegisterTaskExecutorWithRollback(taskExecutor TaskExecutor, subTaskExecutor map[string]SubTaskExecutor, subtaskExecutor map[string]SubTaskExecutor) {
	...
	for k, v := range subtaskExecutor {
		if _, ok := _em.subtaskExecutor[k]; !ok {
