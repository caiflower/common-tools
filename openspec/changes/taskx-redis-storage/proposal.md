...
2. 状态更新延迟高：调度循环中频繁的 `SetState`、`SetWorkerAndStateWithOldWorker` 等 CAS 操作受限于 SQL 行锁竞争
...
- **taskx/dao/**: DAO 接口签名变更（移除 `*bun.Tx` 参数），新增 `store.go` 定义存储抽象接口
...
- **taskx/receiver.go**: `taskReceiver` 需适配新的 DAO 接口签名
- **taskx/backup.go**: 备份逻辑需根据存储后端分支处理
- **taskx/dao/model/**: Model 层保持不变（bun 标签仅 SQL 使用，Redis 用 JSON 序列化）
- **向后兼容**: 现有 SQL DAO 实现（`taskx/dao/task.go` 等）需适配新接口，但内部逻辑不变
